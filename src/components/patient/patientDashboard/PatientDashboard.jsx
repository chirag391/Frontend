...
const PatientDashboard = () => {
  ...
  const getdoctors = async () => {
    try {
      const dataset = await axios.get("https://doctors-backend-e5w1.onrender.com/getapproveddoctors");
        ...
        doctorDetails.map((item, index) => (
          <div key={index}>
            <div className='itemclass'>
              <img className='patientdashboardimage' src={`https://doctors-backend-e5w1.onrender.com/` + item.profile_image} alt={item.name}></img>
